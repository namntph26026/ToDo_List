<template>
    <div class="container" style="text-align: center">
      <div class="content-area" style="position: relative">
        <div class="grid-container">
          ...
          >
            <p class="pin">Results for "{{ query }}" <span id="countinbox"></span></p>
          </div>
          <div class="column"></div>
        </div>
        <div class="grid-container">
          ...
          <div class="column">
            <div
              ...
              "
              v-for="searchResultsUrl in searchResultsUrls"
              :key="searchResultsUrl.id"
            >
              <div class="project-favorite">
                <input
                  ...
                  value=""
                  id="flexCheckDefault"
                  @click="complete(searchResultsUrl.id)"
                />
                <a href="javascript:void(0)" @click="deltail()">
                  <router-link
                    :to="{
                      name: 'backend-search-deltail',
                      params: { id: searchResultsUrl.id },
                      query: {query: query}
                    }"
                  >
                    <div class="detailtasktoday" data-id="" @click="openModal2()">
                      <label class="form-check-label" for="inlineCheckbox1"
                        ><h5>{{ searchResultsUrl.task_name }}</h5> </label
                      ><br />
                      <p>{{ searchResultsUrl.task_description }}</p>
                    </div>
                  </router-link>
                </a>
                <div style="display: flex; margin-left: 60%">
                  <a href="javascript:void(0)" @click="editTask(searchResultsUrl.id)">
                    <button
                      type="button"
                  ...
                  <a href="javascript:void(0)" @click="delTask(searchResultsUrl.id)">
                    <button
                      type="button"
              ...
              <p style="margin-left: 80%;margin-top: -35px;">{{ searchResultsUrl.id_project=== null ? "inbox": searchResultsUrl.project_name }}</p>
              <hr />
            </div>
          </div>
        </div>
        ...
        <div class="grid-container todayabc" :style="{ display: todayabc }">
          ...
          >
            <div>
              ...
              <p class="pwell">
                Well done, trongnam032003! All your <br />
  ...
  <script>
  import { defineComponent, reactive, ref, toRefs, onMounted, watch,computed,watchEffect  } from "vue";
  ...
  export default defineComponent({
    setup() {
      ...
      const getListSearch=() => {
        const token = localStorage.getItem("token");
        axios
        .get(`http://127.0.0.1:8000/api/search?query=${query.value}`,{
            headers: {
              Authorization: `Bearer ${token}`,
            },
          })
          .then((response)=>{
            if(response.status===200){
              searchResultsUrls.value = response.data.data;
