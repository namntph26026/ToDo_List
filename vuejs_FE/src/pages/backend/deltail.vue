              ...
              <a href="" @click="deltail(task.id)">
                <router-link
                  :to="{
                    ...
                    params: {id: task.id_project  ,iddeltail: task.id },
                  }"
                >
...
import TheMenu from "../../components/backend/TheMenu.vue";
...
// import { deltailProject } from "/xampp/htdocs/DUAN_TT_PHP/vuejs_FE/src/components/backend/TheMenu.vue";
// import TheMenu from "../components/backend/TheMenu.vue";
export default defineComponent({
  setup() {
    ...
    //   if (event.target.closest('#myModal2')) {
    //     // Sự kiện click xảy ra bên ngoài vùng modal
    ...
    const id_task_add = ref(route.params.iddeltail);

    // Sử dụng watch để theo dõi thay đổi của tham số id_task trên URL
    watch(
      () => route.params.iddeltail,
      (newIdTask) => {
    ...
    const getTasks = () => {
      const token = localStorage.getItem("token");
      axios
        .get(`http://127.0.0.1:8000/api/task/project/${route.params.id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
    ...
    const deltail = () => {
      const token = localStorage.getItem("token");
      axios
        .get(`http://127.0.0.1:8000/api/task/${route.params.iddeltail}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
    ...
    // onMounted(async () => {
    //   try {
    //     const response = await deltailProject(route.params.id);
    //     projectDeltail = response.data.data;
    ...
    const deltailProject = () => {
      const token = localStorage.getItem("token");
      // alert(route.params.id)
      axios
        .get(`http://127.0.0.1:8000/api/project/${route.params.id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
        .then((response) => {
          ...
          projectDeltail.value = response.data
    ...
    deltailProject();
    // Trong component của router hiện tại
    watchEffect(() => {
      deltailProject();
      deltail();
    });
